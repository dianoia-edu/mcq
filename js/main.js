...
function showSEBQRCode(accessCode, title) {
    ...
    const sebConfigFlexibleUrl = baseUrl + 'seb_config_minimal.php?code=' + accessCode;
    const nameFormUrl = baseUrl + 'name_form.php?code=' + accessCode + '&seb=true';
    
    // KORREKTE EMBEDDED CONFIG URLs (sebs:// Schema für HTTPS) - MINIMAL CONFIG
    const host = window.location.host;
    const path = window.location.pathname.replace(/\/teacher\/.*$|\/[^\/]*$/, '/');
    const sebsEmbeddedUrl = 'sebs://' + host + path + 'seb_config_minimal.php?code=' + accessCode;
...
$(document).on('click', '.download-seb-btn', function() {
    ...
    const currentBaseUrl = window.location.origin + window.location.pathname.replace(/\/teacher\/.*$|\/[^\/]*$/, '/');
        const sebDownloadUrl = currentBaseUrl + 'seb_config_minimal.php?code=' + extractedCode;
